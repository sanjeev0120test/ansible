#!/bin/bash
# Quick connectivity health check against all inventory hosts.
# Pass extra flags: bash scripts/run_ping.sh -v
set -e
...
cd "$REPO_ROOT"
ansible-playbook -i inventory/hosts.autogen.yml playbooks/ping.yml \
